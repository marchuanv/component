...
const { MessageBusMessageSubscription, MessageBus, MessageBusMessage } = require("component.messagebus");
...
Component.prototype.subscribe = async function({ callbackValidate, callback }) {
    for(const { moduleName } of this.config.dependencies){
        const subscriptionMessage = new MessageBusMessageSubscription({ Id: this.name, messageId: moduleName, callback, callbackValidate });
...
Component.prototype.publish = async function({ headers, data, statusCode, statusMessage }) {
    const message = new MessageBusMessage({ Id: this.name, headers, body: data, statusCode, statusMessage, success: false });
    message.validate();
    return await messagebus.publish(message);
};
