...
Component.prototype.notifyDependantComponents = async function(params) {
    if (currentControlId) {
        componentCallstack.unshift({ Id: currentControlId, componentName: this.name, success: controlSuccess });
    }
    const { controlId, results } = await delegate.call({ context: this.name, name: this.config.channel }, params);
    if (!currentControlId){
        componentCallstack.unshift({ Id: controlId, componentName: this.name, success: results.success? true: false });
    } else {
        currentControlId = controlId;
        controlSuccess = results.success? true: false;
    }
    return results;
};
