...
const { MessageBusSubscription, MessageBusMessage, MessageBusMessageStatus, MessageBus } = require("component.messagebus");
...
Component.prototype.subscribe = async function(subscription) {
    if (subscription instanceof MessageBusSubscription) {
        for(const { moduleName } of this.config.dependencies){
            ...
            subscription.channels.push(moduleName);
...
module.exports = { Component, MessageBusSubscription, MessageBusMessage, MessageBusMessageStatus };