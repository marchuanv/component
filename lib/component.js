...
Component.prototype.subscribe = async function(validateCallback, callback) {
    for(const { moduleName } of this.config.dependencies){
        await messagebus.subscribe({ component: moduleName, callback, validateCallback });
    };
};

Component.prototype.publish = async function({ text, headers }) {
    return new Message(await messagebus.publish({ component: this.name, message: { text, headers } }));
};
