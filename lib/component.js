...
Component.prototype.notifyDependantComponents = async function(params) {
    ...
    componentCallstack.unshift({ Id: controlId, componentName: this.name });
    return results;
};

Component.prototype.getCallstack = async function(Id = null, latest = true) {
    let clonedComponentCallstack = utils.getJSONObject(utils.getJSONString(componentCallstack));
    if (Id){
        clonedComponentCallstack = clonedComponentCallstack.filter(cc => cc.Id === Id);
    }
    const stack = [];
