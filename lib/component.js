...
Component.prototype.subscribe = async function(callbackValidate, callback) {
    for(const { moduleName } of this.config.dependencies){
        const message = new MessageBusMessage({ Id: moduleName, headers: {}, body: {}, callbackValidate, callback });
        message.validate();
...
Component.prototype.publish = async function({ headers, data }) {
    const message = new MessageBusMessage({ Id: this.name, headers, body: data });
    message.validate();
    const { headers, body } = await messagebus.publish(message);
