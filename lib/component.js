...
Component.prototype.subscribe = async function(callback) {
    for(const { moduleName } of this.config.dependencies){
        await delegate.register({ context: moduleName, name: this.config.channel, overwriteDelegate: true }, callback);
    };
};

Component.prototype.publish =  async function(params) {
    return await delegate.call({ context: this.name, name: this.config.channel }, params);
};
