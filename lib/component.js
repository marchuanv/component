...
Component.prototype.subscribe = async function({ statusCode, statusMessage, callbackValidate, callback }) {
    for(const { moduleName } of this.config.dependencies){
        const message = new MessageBusMessage({ Id: moduleName, headers: {}, body: {}, callbackValidate, callback, statusCode, statusMessage });
        message.validate();
        await messagebus.subscribe(message);
    };
};
